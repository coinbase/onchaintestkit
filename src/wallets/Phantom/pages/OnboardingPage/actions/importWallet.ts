...
export async function importWallet(
  ...
  try {
    ...
    if (page.isClosed()) {
      throw new Error('Phantom page was closed before wallet import could begin');
    }
    ...
    try {
      ...
      console.warn('[Phantom Import] NetworkIdle wait failed, continuing anyway:', error);
    }
    ...
    console.log("[Phantom Import] Looking for 'I already have a wallet' button...");
    ...
    const confirmPasswordInput = page.getByTestId('onboarding-form-confirm-password-input');
    await confirmPasswordInput.waitFor({ state: 'visible' });
    await confirmPasswordInput.fill(password);

    // Step 8: Wait for and check terms of service checkbox
    const checkbox = page.getByTestId('onboarding-form-terms-of-service-checkbox');
    ...
    if (page.isClosed()) {
      throw new Error("Phantom page was closed while waiting for 'Get Started' button");
    }
    ...
    throw new Error(`Failed to import Phantom wallet: ${(error as Error).message}`);
  }
}
