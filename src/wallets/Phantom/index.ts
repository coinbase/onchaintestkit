...
const WALLET_CONNECTION_ERROR = new Error('Phantom Wallet extension connection not established');
...
export class PhantomWallet extends BaseWallet {
    ...
    try {
      ...
      if (isCI) {
        ...
        const anyExtensionPage = pages.find(
          (p) => p.url().includes(`chrome-extension://${this.extensionId}`) && !p.isClosed(),
        );
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            console.warn(`[Phantom CI] Popup open attempt ${attempt} failed:`, err);
      ...
      if (newPage.isClosed()) {
        console.log('Phantom main popup closed after newPage.goto, skipping waitForLoadState.');
        return;
      }
      await newPage.waitForLoadState('networkidle', { timeout: 15000 });
      this.page = newPage;
  ...
  static async createContext(contextPath: string, slowMo = 0): Promise<BrowserContext> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        const cacheDir = path.join(process.cwd(), 'e2e', '.cache', 'phantom-extension');
        ...
        if (isCI) {
          ...
          console.log(`[Phantom CI] Extension exists: ${fs.existsSync(phantomPath)}`);
        }
  ...
  async handleAction(action: PhantomActionType, options?: ActionOptions): Promise<void> {
