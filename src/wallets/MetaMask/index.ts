...
const WALLET_CONNECTION_ERROR = new Error('Wallet extension connection not established');
...
export class MetaMask extends BaseWallet {
  ...
  static async initialize(
    ...
    const _metamask = new MetaMask(walletConfig, context, metamaskPage, extensionId);
  ...
  static async createContext(contextPath: string, slowMo = 0): Promise<BrowserContext> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        if (attempt > 0) {
          console.log(`Retrying MetaMask setup (attempt ${attempt + 1}/${MAX_RETRIES})...`);
        }

        // Get MetaMask extension path (assumes prepare-metamask.mjs was run first)
        const cacheDir = path.join(process.cwd(), 'e2e', '.cache', 'metamask-extension');
        const metamaskPath = path.join(cacheDir, `metamask-${TARGET_EXTENSION_VERSION}`);
        ...
        try {
          ...
          if (attempt === MAX_RETRIES - 1) {
            ...
            throw new Error(`Failed to initialize MetaMask extension: ${errorMessage}`);
          }
    ...
    throw lastError ?? new Error('Failed to create MetaMask context after multiple attempts');
  }
  ...
  async handleAction(action: MetaMaskActionType, options?: ActionOptions): Promise<void> {
    ...
    switch (action) {
      ...
      case BaseActionType.IMPORT_WALLET_FROM_PRIVATE_KEY:
        await this.homePage.importPrivateKey(additionalOptions.privateKey as string);
        break;
