...
async function processRecoveryPhrase(page: Page, recoveryPhrase: string) {
  ...
  if (phraseTokens.length < 12 || phraseTokens.length > 24) {
    throw new Error(`Recovery phrase must be 12-24 words, got ${phraseTokens.length}`);
  }
  ...
  for (let tokenIndex = 0; tokenIndex < phraseTokens.length; tokenIndex++) {
    ...
    const inputField = page.locator(`[data-testid="import-srp__srp-word-${tokenIndex}"]`);
...
export async function importWallet(page: Page, seedPhrase: string, password: string) {
  ...
  for (const step of onboardingSteps) {
    try {
      ...
      throw new Error(`Wallet import failed at step '${step.name}': ${errorMessage}`);
    }
  }
  ...
  for (let actionIndex = 0; actionIndex < postOnboardingActions.length; actionIndex++) {
