...
export async function addNetwork(page: Page, network: NetworkConfig) {
  ...
  await page.getByTestId('custom-network-chain-id-input').fill(chainId.toString());
  await page.getByTestId('custom-network-currency-symbol-input').fill(symbol);

  if (blockExplorerUrl) {
    await page.getByTestId('custom-network-block-url-input').fill(blockExplorerUrl);
  }
  ...
  const testnetErrorMessage = await page
    .getByText(`Testnet chain id: ${chainId} already exists. Please enable testnets from settings.`)
    .isVisible()
    .catch(() => false);
  const networkErrorMessage = await page
    .getByText(`Chain id: ${chainId} already exists. Select from default networks.`)
    .isVisible()
    .catch(() => false);
