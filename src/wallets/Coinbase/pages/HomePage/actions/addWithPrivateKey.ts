...
export async function addWithPrivateKey(
  ...
  try {
    ...
    await page.getByTestId('add-and-manage-wallets--importNewSecret-cell-pressable').click();
  } catch (error) {
  ...
  try {
    await page.waitForSelector('text="Wallet imported. Balances will appear momentarily."', {
      timeout: 5000,
    });
